	...
	public void actionPerformed(ActionEvent ae) 
     {
		...
		if(ae.getSource()== btn_delete)
		{
			...
			{
				checkEngineerAssociated();
				if(!checkAlottedServices())
				deleteDetails();
				else
					JOptionPane.showMessageDialog(this, Name+"/"+Engineerid+" is already working on this applianceid", "Error", JOptionPane.ERROR_MESSAGE);
		...
		{
			int selection =	JOptionPane.showConfirmDialog(this, "It will delete the appliance permanently" , "Confirmation",JOptionPane.WARNING_MESSAGE);
		...
		public void checkEngineerAssociated()
		{
			 String strcheck = "select engineerid,name from engineerdetails where applianceid=?";
			  
			  try {
				psread = con.prepareStatement(strcheck);
				String applianceid = (String)cmb_applianceid.getSelectedItem();
				psread.setString(1, applianceid);
				rsread = psread.executeQuery();
				
				while(rsread.next())
				{
					
					Engineerid = rsread.getString("engineerid");
					Name = rsread.getString("name");
		...
		public boolean checkAlottedServices()
		{
			
			String strcheck = "select requestid from allotservice where engineerid=?";
