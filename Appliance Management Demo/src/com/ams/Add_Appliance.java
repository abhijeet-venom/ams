...
public class Add_Appliance extends JFrame implements ActionListener,WindowListener,KeyListener
{
   ...
   public boolean checkApplianceId()
   {
		...
		if(caption.equalsIgnoreCase("Enroll"))
			{
		     ...
		     { 
			     ...
			     {
			    	 if(checkApplianceName())
			    	 {
			    		 JOptionPane.showMessageDialog(this, "Appliance already exist", "Error",JOptionPane.ERROR_MESSAGE);	
				         txt_name.grabFocus();
					     txt_name.selectAll();
	...
	public boolean checkApplianceName()
	{
		String strcheck = "select name from appliancedetails";
		  ...
		  try {
			...
			while(rsread.next())
			{
				...
				String ApplianceName = rsread.getString("name");
				
			  if(name.equalsIgnoreCase(ApplianceName))
			  {
				  return true;
